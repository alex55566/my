...
export function createInput() {
  ...
  let inputPlacehold = ["Номер карты", "Дата окончания карты", "CVC", "E-mail"];
...
inputEndCard.addEventListener("blur", function () {
  ...
  try {
    v8n().minLength(4).maxLength(4).check(dateAndYear);
...
function compareDateLogic(date) {
  let validationDate = moment(
    `${date.substring(0, 2)} ${date.substring(date.length, 2)}`,
    "MM YYYY"
  ).isValid();
  if (validationDate) {
    let totalDate = moment(
      `${date.substring(0, 2)} ${20 + date.substring(date.length, 2)}`,
      "MM YYYY"
    )._i;
...
function compareDateIntime(date) {
  ...
  if (inputDate > lastdate) {
    renderError(
      "Ошибка валидации: дата окончания карты не может быть больше больше +4лет от текущей даты"
    );
    clearDate();
    checkBtn();
    return;
...
inputCVC.addEventListener("blur", function () {
  try {
    checkCVC([this.value, "validation"]);
    checkBtn();
  } catch (ex) {
    this.value = "";
    renderError(`Ошибка валидации: не соблюдается ${ex.rule.name}`);
...
inputEmail.addEventListener("blur", function () {
  try {
    v8n().minLength(10).includes("@").includes(".").check(this.value);
    checkBtn();
  } catch (ex) {
    this.value = "";
    renderError(`Ошибка валидации: не соблюдается ${ex.rule.name}`);
